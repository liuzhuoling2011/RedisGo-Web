...
export default {
  ...
  methods: {
    ...mapMutations(["initWS"]),
    resetCharts() {
      option.dataset.source = this.chart_data[this.redis_id]
    ...
    updateCharts() {
      if (this.last_time[this.redis_id] === undefined) {
        this.last_time[this.redis_id] = this.chart_id_data[0]
        option.dataset.source = this.chart_data[this.redis_id]
      } else if (this.last_time[this.redis_id] !== this.chart_id_data[0]) {
        // 貌似是引用的, 这里无需在push
